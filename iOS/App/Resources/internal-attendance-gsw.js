...
(function(){
  ...
  var I18N={
    withFood:'Mit Ässe',
    withoutFood:'Ohni Ässe',
    notSelected:'Nüt gwählt',
    notComing:'Ich chumme nöd',
    standard:'Standard',
    remove:'Entferne',
    noSelection:'Für dä Termin isch nüt gwählt',
    rebuild:'Us em Initiallink neu ufbaue',
    cleanupTitle:'Aasicht bereinige',
    cleanupNote:'Wenn d Personeaasicht feschthängt oder z viel Persone glade worde sind, chasch sie us em gspeicherte persönliche Initiallink komplett neu ufbaue.',
    rebuildConfirm:'No einisch tippe: würkli neu ufbaue',
    initialUnavailable:'Initiallink nöd verfüegbar',
    managePeople:'Persone verwalte',
    close:'Schliesse',
    addPerson:'Person hinzuefüege:',
    addUnavailable:'Hinzuefüege isch bi däm Sitestand nöd verfüegbar. Vorhandeni Zusatzpersone chönd witerhin entfernt werde oder d Aasicht cha unde us em Initiallink neu ufbaut werde.',
    removeNote:'Entferne aktualisiert d Personelischte automatisch.',
    currentPeople:'Aktuelli Persone',
    appointment:'Termin',
    removeFromView:'Person us de Aasicht entferne: ',
    addPersonAria:'Person hinzuefüege',
    participant:'Person',
    restoreSaved:'Gspeichereti Persone wiederherställe'
  };
  ...
  var decorateDayMeta=function(meta){
    ...
    Array.from(meta.querySelectorAll('input[type=text],input:not([type])')).forEach(function(input){
      ...
      var value=((input.value||input.getAttribute('value')||'')+'').replace(/ +/g,' ').trim();
      if(!value)return;
    ...
    Array.from(meta.childNodes).forEach(function(node){
      ...
      var raw=text(node).replace(/ +/g,' ').trim();
