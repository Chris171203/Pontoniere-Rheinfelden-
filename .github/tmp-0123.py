...
old_slots='List<WeatherDaily.Slot> slots=WeatherDaily.slots(hours,day,10,14,18);'
if s.count(old_slots)!=2: raise SystemExit(f'weather slot calls: expected 2, got {s.count(old_slots)}')
s=s.replace(old_slots,'List<WeatherDaily.Slot> slots=WeatherDaily.slots(hours,day,6,12,18);',2)
