#!/usr/bin/env python3
"""Select two different available iPhone sizes on the newest installed iOS runtime."""
import argparse
import json
import re
import subprocess
...
def select(devices, profile):
    ...
    raise RuntimeError('No installed iOS runtime has both a compact/standard and a Max/Plus iPhone. Install simulator devices in Xcode.')
...
def main():
    ...
    runtime, device = select(listing['devices'], args.profile)
    print(f"{args.profile}: {device['name']} / {runtime} / {device['udid']}", file=sys.stderr)
    print(device['udid'])
