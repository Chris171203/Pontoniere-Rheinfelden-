...
old_remove = """                  var removePersonFromMatrix=function(name){
...
"""
new_remove = old_remove + """                  var appendManagedPerson=function(list,state,personName){
...
"""
s = replace_once(s, old_remove, new_remove, 'people sync helpers')

start = s.index("                  var findPersonTools=function(toolInfo,state){")
end = s.index("\n\n                  var saveViewState=function(){", start)
new_tools = """                  var findPersonTools=function(toolInfo,state){
...
s = replace_once(
    ...
    'bind source observer'
)
path.write_text(s)

test_path = Path('Android/app/src/test/java/ch/pfvr/internapp/InternalAttendanceSkinTest.java')
t = test_path.read_text()
t = replace_once(t, '        assertTrue(script.contains("body.appendChild(select)"));\n', '        assertFalse(script.contains("body.appendChild(select)"));\n', 'select move test')
marker = """    @Test public void generatedScriptPersistsRestoresRemovesAndLabelsParticipants(){
"""
new_test = """    @Test public void generatedScriptKeepsOriginalPersonControlInWebsiteContextAndSyncsNewRows(){
...
""" + marker
t = replace_once(t, marker, new_test, 'new proxy/sync test')
